...
function normalizeText(s){ return s.replace(/^\uFEFF/,'').replace(/\r\n?/g,'\n').trimEnd(); }
function fileKey(st){ return ${st.size}-${st.mtimeMs}; }

async function listFilesRecursive(dir){
  const out=[]; async function walk(c){
    const es=await fs.readdir(c,{withFileTypes:true});
    for(const e of es){ const p=path.join(c,e.name); e.isDirectory()?await walk(p):out.push(p); }
  } await walk(dir); return out;
}

async function buildIndex(){
  if(!fssync.existsSync(TEXT_DIR)) await fs.mkdir(TEXT_DIR,{recursive:true});
  const previous=(await safeReadJson(OUT_FILE)) ?? {version:1,items:[],stats:{}};
  const prevMap=new Map(previous.items.map(it=>[it.relPath,it]));
  let files=[]; try{ files=(await listFilesRecursive(TEXT_DIR)).filter(isAllowedFile); }catch{}
  const items=[]; const errors=[]; let reused=0;

  const queue=[...files];
  const workers=Array.from({length:CONCURRENCY},()=> (async function worker(){
    while(queue.length){
      const abs=queue.pop(); const rel=path.relative(process.cwd(),abs);
      try{
        const st=await fs.stat(abs);
        if(st.size>MAX_BYTES){ errors.push({relPath:rel,reason:excede ${MAX_BYTES} bytes}); continue; }
        const key=fileKey(st); const prev=prevMap.get(rel);
        if(prev && prev.validation?.fileKey===key){ items.push(prev); reused++; continue; }
        const buf=await fs.readFile(abs); const hash=sha1(buf);
        const content=normalizeText(buf.toString('utf8'));
        items.push({ id:hash,name:path.basename(abs),relPath:rel,size:st.size,
  ...
  const payload={ version:1, generatedAt:new Date().toISOString(),
    baseDir:path.relative(process.cwd(),TEXT_DIR),
    items:items.sort((a,b)=>a.relPath.localeCompare(b.relPath)),
    stats:{ totalFilesSeen:files.length,indexed:items.length,reused,failed:errors.length,
            maxBytes:MAX_BYTES,concurrency:CONCURRENCY },
    errors };
...
app.get('/index', async (_req,res)=> {
  ...
  if(!data) return res.status(404).json({error:'index.json no existe todavía. Visita /reindex.'});
  res.json(data);
});
